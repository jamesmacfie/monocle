...
Firefox differs from Chrome in important ways — see `references/firefox.md` for full details. Key gotchas:

- WXT defaults Firefox to **MV2**; pass `--mv3` explicitly (`wxt build -b firefox --mv3`).
- Firefox MV3 uses **event pages** (`background.scripts`), not service workers — WXT generates the right manifest key per target.
- `browser_specific_settings.gecko.id` is **required** to sign/publish MV3 extensions.
- Firefox MV3 treats `host_permissions` as **optional** — users can revoke them anytime; always check `permissions.contains()` before privileged work.
- AMO requires a **source code ZIP** alongside the extension ZIP (`wxt zip -b firefox` produces both).
- Firefox-only APIs worth knowing: `contextualIdentities` (containers), `sidebarAction`, `menus`, `find`, `search`, `theme`.
