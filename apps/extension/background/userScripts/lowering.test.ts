// Architecture: background tests. The lockstep corollary for user scripts:
...
const CONTENT_STEPS: UserScriptStep[] = [
...
const ENGINE_STEPS: UserScriptStep[] = [
...
describe("lowering", () => {
  ...
  it("interpolates fill text and stamps scope keys", () => {
    ...
    expect(hide).toMatchObject({ scopeKey: "userscript-script-1" })
  })
...
describe("loop retargeting", () => {
  ...
  it("pins matching targets (including within scopes) to the iteration index", () => {
    ...
    const direct = retargetForLoopIteration(
      { op: "click", target: css(".row") } as UserScriptStep,
      loopSelector,
      2,
    )
    ...
    const untouched = retargetForLoopIteration(
      { op: "click", target: css(".unrelated") } as UserScriptStep,
      loopSelector,
      1,
    )
