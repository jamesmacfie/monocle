...
// (background/automations/triggerEngine.ts): arming specs are scoped by
...
const runAutomationMock = vi.fn()

vi.mock("./engine", () => ({
  runAutomation: (automationId: string, input: unknown) =>
    runAutomationMock(automationId, input),
}))
...
beforeEach(() => {
  fakeBrowser.reset()
  installBrowserStubs()
  runAutomationMock.mockReset().mockResolvedValue({
    success: true,
    completedSteps: 1,
  })
})
...
const addPageTriggerScript = (
  overrides: Partial<Parameters<typeof addAutomation>[0]> = {},
) =>
  addAutomation({
    schemaVersion: 1,
    name: "Dismiss banner",
...
describe("handleTriggerFired", () => {
  it("re-validates and runs against the sender tab and sender URL", async () => {
    ...
    expect(runAutomationMock).toHaveBeenCalledWith(
      script.id,
