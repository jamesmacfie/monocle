// Architecture: content layer. The page-side automation trigger service:
// pulls armed trigger specs for the current URL from the background
// (`get-automation-triggers`), then watches the page and reports fires
// back (`automation-trigger-fired`). Content holds NO script steps and
// executes nothing on its own — the background re-validates every fire
// (background/automations/triggerEngine.ts) and the engine enforces
...
const armTriggers = (specs: AutomationPageTriggerSpec[]): void => {
  state.urlTriggers = []
...
const fireUrlTriggers = (kind: "load" | "spa"): void => {
  ...
  for (const trigger of state.urlTriggers) {
    ...
    if (trigger.delayMs > 0) {
      setTimeout(
        () => reportFire(trigger.automationId, "urlMatch"),
        trigger.delayMs,
      )
    } else {
      reportFire(trigger.automationId, "urlMatch")
 ...
 * refuses such URLs.
 */
export const initializeAutomationTriggers = (): void => {
