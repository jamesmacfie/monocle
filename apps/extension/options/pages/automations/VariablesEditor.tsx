...
// the draft's declared variables (shared/types/automations.ts
// AutomationVarDef): name plus kind — a literal value, a snippet reference
...
import type { AutomationVarDef, Snippet } from "../../../shared/types"
...
export function VariablesEditor({
  ...
  const setKind = (index: number, kind: AutomationVarDef["kind"]) => {
    const def: AutomationVarDef =
      kind === "literal"
        ? { kind: "literal", value: "" }
        : kind === "snippet"
      ...
      {rows.map((row, index) => (
        <div key={index} className="flex flex-wrap items-center gap-2">
          ...
          <Select
            ...
            value={row.def.kind}
            onChange={(event) =>
              setKind(index, event.target.value as AutomationVarDef["kind"])
            }
          >
