import type { UserScriptCondition, UserScriptStep } from "../types"
import { collectTemplateReferences } from "./user-script-template"

export const collectConditionInterpolatableValues = (
  condition: UserScriptCondition,
): string[] => {
  const values: string[] = []
  const walk = (current: UserScriptCondition): void => {
...
export const walkUserScriptSteps = (
  ...
  for (const step of steps) {
    visit(step)
    if (step.op === "branch") {
      walkUserScriptSteps(step.then, visit)
      if (step.else) {
        walkUserScriptSteps(step.else, visit)
      }
    } else if (step.op === "forEach" || step.op === "while") {
      walkUserScriptSteps(step.steps, visit)
    }
  }
}
...
export const collectInlineSnippetReferences = (
  ...
  walkUserScriptSteps(steps, (step) => {
