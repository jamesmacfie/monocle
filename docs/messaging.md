...
| `monocle-automation-generation-settings-get` | options -> bg | `{}` | `{ hasApiKey, model }` | `background/messages/automationGeneration.ts` | Return status only; never return the stored key. |
| `monocle-automation-generation-key-set` | options -> bg | `{ apiKey }` | `{ saved: true, status }` or `{ error }` | `background/messages/automationGeneration.ts` | Store a trimmed local BYOK value in the background-owned generation record. |
| `monocle-automation-generation-key-clear` | options -> bg | `{}` | `{ cleared: true, status }` or `{ error }` | `background/messages/automationGeneration.ts` | Remove the local OpenAI API key. |
| `monocle-automation-generate` | options -> bg | `{ generationId, request }` | `AutomationGenerationResult` | `background/messages/automationGeneration.ts` | Recheck origin/Firefox consent, call OpenAI, normalize, and validate a draft; never persist it. |
| `monocle-automation-generation-cancel` | options -> bg | `{ generationId }` | `{ cancelled: boolean }` | `background/messages/automationGeneration.ts` | Abort the matching in-flight request; unknown/completed ids return false. |
...
| `monocle-surface-action` | content/new-tab -> bg | `{ ownerId, surfaceId, actionId, value?, selection? }` | `{ success: boolean, error?, result? }` | `background/messages/surfaceAction.ts`, `surfaceAction` | Report a surface gesture; `dismiss` removes the surface, feature/command actions route to their registered owners, and `automation:<id>` actions require a top-frame sender plus an active inline surface before a fresh nested action run. |
...
- **`monocle-surface-action`** `{ ownerId, surfaceId, actionId, value?, selection? }` → `{ success, error?, result? }` (handler: `background/messages/surfaceAction.ts`) — a user interaction reported by the host. `dismiss` is universal. Feature and command owners retain their registered routes. For `automation:<id>`, the background rejects missing/child-frame senders, derives the real URL from `MessageSender`, re-queries the active render-only surface/action, rereads the current Automation document, and runs only the selected nested steps in that tab. Content never supplies a URL, destination, payload, or executable step.
