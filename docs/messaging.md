...
| `monocle-copyToClipboard` | bg -> tab | `{ message }` | many command executors (`copyUuidV4.ts`, `copyCurrentTabUrl.ts`, `snippets.ts`, …) | `shared/components/Listeners/CopyToClipboardListener.tsx` (`navigator.clipboard.writeText`) |
