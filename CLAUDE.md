...
| Workflow automation | Working with review notes | The full step vocabulary (click/wait/fill/type/key/select/check/uncheck/submit/focus/blur/scroll/hover/getText/removeElement/hideElement/injectCss) is implemented in `content/workflow/`, schema-accepted, and covered by linkedom tests; privileged ops are automation engine ops. `click`/`submit` carry an optional `expectNavigation` orchestration hint that is **stripped during lowering** (not content behaviour) and consumed only by the automation engine. Manual fixture-page smoke is still needed. |
| Automations (Automations) | Working with review notes | Declarative automation documents: schema + caps validation, `monocle-automations` storage, background engine (interpolation, segmentation, lowering, branch/forEach/while, runCommand policy), manual/urlMatch/elementAppears/interval/schedule/onStartup triggers, generated palette commands, and the options builder with validated import/export. Cross-navigation multi-step flows (e.g. signups) use `expectNavigation` on a `click`/`submit`: the engine ends that segment, waits navigation-tolerantly for the page load (tolerating the torn-down content script's lost response), and runs the next segment fresh — the page-driven sibling of the `navigate` engine op, with variables carrying across pages. Storage, validation, lowering (incl. expectNavigation segment-ending + flag stripping), conditions, policy, command generation, engine (incl. navigation-tolerant segments), and trigger-engine behavior have focused tests; manual browser smoke (triggers, schedules, builder, multi-page navigation) is still needed. |
...
- `pnpm test` passes cleanly (exit 0, 617 tests) with focused command-system,
  ...
  (storage/validation/lowering incl. expectNavigation segment-ending + flag
  stripping/conditions/policy/commands/engine incl. navigation-tolerant
  segments/trigger-engine/scheduled-alarm sync), template-interpolation,
  new-tab/theme/background,
