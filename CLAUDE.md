...
- `docs/element-hider.md`: Element Hider, the third feature — the first consumer
  ...
  selection reported back via `surface-action`), generic `surface-action` owner
...
- `pnpm test` passes cleanly (exit 0, 577 tests) with focused command-system,
  ...
  `UserScriptSchema`, handleAction picked/delete) and picker selector generation,
  surface-action owner routing,
...
2. The one generic `SurfaceHost` (mounted in the closed content shadow root with
   ...
   `get-surfaces {url}` on mount/navigation/broadcast and renders the surfaces of
   ...
   pick-mode and reports the clicked element via `surface-action`.
3. `surface-action` handling: `dismiss` removes the surface (universal); any
   other action routes to the owner — a feature's `handleAction` (same path as
   `execute-feature-action`) or a command's registered handler
