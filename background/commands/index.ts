...
import { addToRecentCommandIds, getRecentCommandIds, getRankedCommandIds } from "./usage";
import { getFavoriteCommandIds, toggleFavoriteCommandId, clearFavoritesCommand } from "./favorites";
...
export const getCommands = async (): Promise<{
  ...
  const allCommandsUnfiltered = [...browserCommands, ...toolCommands, clearRecentsCommand, clearFavoritesCommand, debug];
  console.debug("[Commands] All commands unfiltered:", allCommandsUnfiltered.length, allCommandsUnfiltered.map(c => c.id));
  ...
  const findCommandById = (commands: Command[], id: string): Command | undefined => {
    ...
    for (const command of commands) {
      ...
      if (favoriteCommandIds.includes(command.id)) {
        ...
        if (parentName) {
          ...
          const modifiedCommand: Command = {
            ...command,
            name: Array.isArray(resolvedName) ? resolvedName : [resolvedName!, parentName],
          };
      ...
      if ("commands" in command) {
        try {
          ...
          const parentNameString = Array.isArray(commandName) ? commandName[0] : commandName!;
  ...
  const remainingCommands = allCommands.filter(command => !addedCommandIds.has(command.id));
...
export const findCommand = async (
  ...
  for (const command of commands) {
    ...
    if (command.actions && Array.isArray(command.actions)) {
      const foundInActions = await findCommand(command.actions, commandId, context);
      if (foundInActions) {
...
export const executeCommand = async (
  ...
  if (id.startsWith("toggle-favorite-")) {
    ...
    const newState = await toggleFavoriteCommandId(originalCommandId);
    return;
  }
  ...
  if (commandToRun) {
    ...
    if ("run" in commandToRun) {
      ...
      console.error(`[ExecuteCommand] Command found but has no run method: ${id}`);
...
export const commandsToSuggestions = async (
  ...
  return await Promise.all(
    commands.map(async (command) => {
      ...
      let actions: CommandSuggestion[] | undefined = undefined;
      if (command.actions && Array.isArray(command.actions)) {
        const commandName = await resolveAsyncProperty(command.name, context);
        // Pass the resolved name as parentName for actions
        const resolvedParentName = Array.isArray(commandName) ? commandName[0] : commandName!;
        actions = await commandsToSuggestions(command.actions, context, resolvedParentName);
      }
      ...
      const displayName = parentName && favoriteCommandIds.includes(command.id)
        ? Array.isArray(resolvedName) ? resolvedName : [resolvedName!, parentName]
        : resolvedName;
      ...
      const toggleFavoriteAction: CommandSuggestion = {
        ...
        description: isFavorite ? "Remove this command from favorites" : "Add this command to favorites",
      ...
      const allActions = actions ? [...actions, toggleFavoriteAction] : [toggleFavoriteAction];
      ...
      return {
        ...
        modifierActionLabel: await resolveModifierActionLabels(command, context),
        actions: allActions,
