...
const calculateTimeBoost = (hourlyUsage: number[], currentHour: number): number => {
  let timeScore = 0;
...
export const calculateCommandScore = (stats: CommandUsageStats, currentHour: number): number => {
...
export const getCommandUsageStats = async (commandId: string): Promise<CommandUsageStats> => {
  const usageData = await loadUsageData();
  return usageData.commandStats[commandId] || createEmptyStats(commandId);
};

// Get all command usage stats
export const getAllUsageStats = async (): Promise<Record<string, CommandUsageStats>> => {
  const usageData = await loadUsageData();
...
export const addToRecentCommandIds = async (commandId: string): Promise<void> => {
