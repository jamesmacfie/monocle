...
const handleGetCommands = async (message: GetCommandsMessage) => {
  ...
  const { favorites: cmdFavorites, recents: cmdRecents, suggestions: cmdSuggestions } = await getCommandsFromBackground();
  ...
  const suggestions = await commandsToSuggestions(cmdSuggestions, message.context);
